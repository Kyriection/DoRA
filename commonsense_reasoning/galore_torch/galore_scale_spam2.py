    ...
    def step(self, closure: Callable = None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                # apply spiking clipping
                if state["step"] > 1:
                    mask = (grad**2) > (self.threshold * exp_avg_sq)
                    if mask.sum() > 0:
                        grad[mask] = grad[mask].sign() * torch.sqrt(exp_avg_sq[mask] * self.threshold) # earsing spiking gradient
